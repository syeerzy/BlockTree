...
namespace BlockTree.UI
{
    internal partial class InvokeContractDialog : Form
    {
        ...
        private static readonly Fixed10 net_fee = Fixed10.FromDecimal(0.001m);
        ...
        {
            Fixed10 fee = tx.Gas.Equals(Fixed10.Zero) ? net_fee : Fixed10.Zero;
        ...
        public InvocationTransaction GetTransaction(UInt160 change_address, Fixed10 fee)
        {
            return Program.CurrentWallet.MakeTransaction(new InvocationTransaction
            {
                Version = tx.Version,
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            sb.AppendLine($"Evaluation Stack: {new JArray(engine.CurrentContext.EvaluationStack.Select(p => p.ToParameter().ToJson()))}");
            textBox7.Text = sb.ToString();
            if (!engine.State.HasFlag(VMState.FAULT))
            {
                tx.Gas = engine.GasConsumed - Fixed10.FromDecimal(10);
                if (tx.Gas < Fixed10.Zero) tx.Gas = Fixed10.Zero;
                tx.Gas = tx.Gas.Ceiling();
                Fixed10 fee = tx.Gas.Equals(Fixed10.Zero) ? net_fee : tx.Gas;
                label7.Text = fee + " gas";
