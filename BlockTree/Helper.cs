...
namespace BlockTree
{
    public static class Helper
    {
        ...
        public static Fixed10 Sum(this IEnumerable<Fixed10> source)
        {
            long sum = 0;
            checked
            {
                foreach (Fixed10 item in source)
                {
                    sum += item.value;
                }
            }
            return new Fixed10(sum);
        }

        public static Fixed10 Sum<TSource>(this IEnumerable<TSource> source, Func<TSource, Fixed10> selector)
        {
