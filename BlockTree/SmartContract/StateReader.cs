...
using VMBoolean = BlockTree.VM.Types.Boolean;
...
namespace BlockTree.SmartContract
{
    public class StateReader : InteropService, IDisposable
    {
        ...
        public StateReader()
        {
            ...
            //BlockTree Specified
            Register("BlockTree.Blockchain.GetAccount", Blockchain_GetAccount);
            Register("BlockTree.Blockchain.GetValidators", Blockchain_GetValidators);
            Register("BlockTree.Blockchain.GetAsset", Blockchain_GetAsset);
            Register("BlockTree.Header.GetVersion", Header_GetVersion);
            Register("BlockTree.Header.GetMerkleRoot", Header_GetMerkleRoot);
            Register("BlockTree.Header.GetConsensusData", Header_GetConsensusData);
            Register("BlockTree.Header.GetNextConsensus", Header_GetNextConsensus);
            Register("BlockTree.Transaction.GetType", Transaction_GetType);
            Register("BlockTree.Transaction.GetAttributes", Transaction_GetAttributes);
            Register("BlockTree.Transaction.GetInputs", Transaction_GetInputs);
            Register("BlockTree.Transaction.GetOutputs", Transaction_GetOutputs);
            Register("BlockTree.Transaction.GetReferences", Transaction_GetReferences);
            Register("BlockTree.Transaction.GetUnspentCoins", Transaction_GetUnspentCoins);
            Register("BlockTree.InvocationTransaction.GetScript", InvocationTransaction_GetScript);
            Register("BlockTree.Attribute.GetUsage", Attribute_GetUsage);
            Register("BlockTree.Attribute.GetData", Attribute_GetData);
            Register("BlockTree.Input.GetHash", Input_GetHash);
            Register("BlockTree.Input.GetIndex", Input_GetIndex);
            Register("BlockTree.Output.GetAssetId", Output_GetAssetId);
            Register("BlockTree.Output.GetValue", Output_GetValue);
            Register("BlockTree.Output.GetScriptHash", Output_GetScriptHash);
            Register("BlockTree.Account.GetScriptHash", Account_GetScriptHash);
            Register("BlockTree.Account.GetVotes", Account_GetVotes);
            Register("BlockTree.Account.GetBalance", Account_GetBalance);
            Register("BlockTree.Asset.GetAssetId", Asset_GetAssetId);
            Register("BlockTree.Asset.GetAssetType", Asset_GetAssetType);
            Register("BlockTree.Asset.GetAmount", Asset_GetAmount);
            Register("BlockTree.Asset.GetAvailable", Asset_GetAvailable);
            Register("BlockTree.Asset.GetPrecision", Asset_GetPrecision);
            Register("BlockTree.Asset.GetOwner", Asset_GetOwner);
            Register("BlockTree.Asset.GetAdmin", Asset_GetAdmin);
            Register("BlockTree.Asset.GetIssuer", Asset_GetIssuer);
            Register("BlockTree.Contract.GetScript", Contract_GetScript);
            Register("BlockTree.Contract.IsPayable", Contract_IsPayable);
            Register("BlockTree.Storage.Find", Storage_Find);
            Register("BlockTree.Enumerator.Create", Enumerator_Create);
            Register("BlockTree.Enumerator.Next", Enumerator_Next);
            Register("BlockTree.Enumerator.Value", Enumerator_Value);
            Register("BlockTree.Enumerator.Concat", Enumerator_Concat);
            Register("BlockTree.Iterator.Create", Iterator_Create);
            Register("BlockTree.Iterator.Key", Iterator_Key);
            Register("BlockTree.Iterator.Keys", Iterator_Keys);
            Register("BlockTree.Iterator.Values", Iterator_Values);

            #region Aliases
            Register("BlockTree.Iterator.Next", Enumerator_Next);
            Register("BlockTree.Iterator.Value", Enumerator_Value);
            #endregion

            #region Old APIs
            Register("BlockTree.Runtime.GetTrigger", Runtime_GetTrigger);
            Register("BlockTree.Runtime.CheckWitness", Runtime_CheckWitness);
            Register("BlockTree.Runtime.Notify", Runtime_Notify);
            Register("BlockTree.Runtime.Log", Runtime_Log);
            Register("BlockTree.Runtime.GetTime", Runtime_GetTime);
            Register("BlockTree.Runtime.Serialize", Runtime_Serialize);
            Register("BlockTree.Runtime.Deserialize", Runtime_Deserialize);
            Register("BlockTree.Blockchain.GetHeight", Blockchain_GetHeight);
            Register("BlockTree.Blockchain.GetHeader", Blockchain_GetHeader);
            Register("BlockTree.Blockchain.GetBlock", Blockchain_GetBlock);
            Register("BlockTree.Blockchain.GetTransaction", Blockchain_GetTransaction);
            Register("BlockTree.Blockchain.GetTransactionHeight", Blockchain_GetTransactionHeight);
            Register("BlockTree.Blockchain.GetContract", Blockchain_GetContract);
            Register("BlockTree.Header.GetIndex", Header_GetIndex);
            Register("BlockTree.Header.GetHash", Header_GetHash);
            Register("BlockTree.Header.GetPrevHash", Header_GetPrevHash);
            Register("BlockTree.Header.GetTimestamp", Header_GetTimestamp);
            Register("BlockTree.Block.GetTransactionCount", Block_GetTransactionCount);
            Register("BlockTree.Block.GetTransactions", Block_GetTransactions);
            Register("BlockTree.Block.GetTransaction", Block_GetTransaction);
            Register("BlockTree.Transaction.GetHash", Transaction_GetHash);
            Register("BlockTree.Storage.GetContext", Storage_GetContext);
            Register("BlockTree.Storage.GetReadOnlyContext", Storage_GetReadOnlyContext);
            Register("BlockTree.Storage.Get", Storage_Get);
            Register("BlockTree.StorageContext.AsReadOnly", StorageContext_AsReadOnly);
            #endregion
        }
        ...
        protected virtual bool Account_GetBalance(ExecutionEngine engine)
        {
            if (engine.CurrentContext.EvaluationStack.Pop() is InteropInterface _interface)
            {
                ...
                if (account == null) return false;
                Fixed10 balance = account.Balances.TryGetValue(asset_id, out Fixed10 value) ? value : Fixed10.Zero;
