...
namespace BlockTree.SmartContract
{
    public class StateMachine : StateReader
    {
        ...
        public StateMachine(Block persisting_block, DataCache<UInt160, AccountState> accounts, DataCache<UInt256, AssetState> assets, DataCache<UInt160, ContractState> contracts, DataCache<StorageKey, StorageItem> storages)
        {
            ...
            //BlockTree Specified
            Register("BlockTree.Asset.Create", Asset_Create);
            Register("BlockTree.Asset.Renew", Asset_Renew);
            Register("BlockTree.Contract.Create", Contract_Create);
            Register("BlockTree.Contract.Migrate", Contract_Migrate);

            #region Old APIs
            Register("BlockTree.Contract.GetStorageContext", Contract_GetStorageContext);
            Register("BlockTree.Contract.Destroy", Contract_Destroy);
            Register("BlockTree.Storage.Put", Storage_Put);
            Register("BlockTree.Storage.Delete", Storage_Delete);
            #endregion
        }
        ...
        private bool Asset_Create(ExecutionEngine engine)
        {
            ...
            Fixed10 amount = new Fixed10((long)engine.CurrentContext.EvaluationStack.Pop().GetBigInteger());
            if (amount == Fixed10.Zero || amount < -Fixed10.Satoshi) return false;
            if (asset_type == AssetType.Invoice && amount != -Fixed10.Satoshi)
                return false;
            ...
            if (amount != -Fixed10.Satoshi && amount.GetData() % (long)Math.Pow(10, 8 - precision) != 0)
                return false;
