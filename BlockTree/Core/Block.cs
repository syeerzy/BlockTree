...
namespace BlockTree.Core
{
    ...
    public class Block : BlockBase, IInventory, IEquatable<Block>
    {
        ...
        public static Fixed10 CalculateNetFee(IEnumerable<Transaction> transactions)
        {
            ...
            Fixed10 amount_in = ts.SelectMany(p => p.References.Values.Where(o => o.AssetId == Blockchain.UtilityToken.Hash)).Sum(p => p.Value);
            Fixed10 amount_out = ts.SelectMany(p => p.Outputs.Where(o => o.AssetId == Blockchain.UtilityToken.Hash)).Sum(p => p.Value);
            Fixed10 amount_sysfee = ts.Sum(p => p.SystemFee);
