...
namespace BlockTree.Core
{
    public class StateDescriptor : ISerializable
    {
        ...
        private Fixed10 GetSystemFee_Validator()
        {
            switch (Field)
            {
                case "Registered":
                    if (Value.Any(p => p != 0))
                        return Fixed10.FromDecimal(1000);
                    else
                        return Fixed10.Zero;
                default:
        ...
        private bool VerifyAccountState()
        {
            switch (Field)
            {
                case "Votes":
                    ...
                    if (pubkeys.Length > 0)
                    {
                        if (account.GetBalance(Blockchain.GoverningToken.Hash).Equals(Fixed10.Zero)) return false;
