...
namespace BlockTree.Core
{
    ...
    public abstract class Transaction : IEquatable<Transaction>, IInventory
    {
        ...
        private Fixed10 _network_fee = -Fixed10.Satoshi;
        public virtual Fixed10 NetworkFee
        {
            get
            {
                if (_network_fee == -Fixed10.Satoshi)
                {
                    Fixed10 input = References.Values.Where(p => p.AssetId.Equals(Blockchain.UtilityToken.Hash)).Sum(p => p.Value);
                    Fixed10 output = Outputs.Where(p => p.AssetId.Equals(Blockchain.UtilityToken.Hash)).Sum(p => p.Value);
        ...
        /// 系统费用
        /// </summary>
        public virtual Fixed10 SystemFee => Settings.Default.SystemFee.TryGetValue(Type, out Fixed10 fee) ? fee : Fixed10.Zero;
        ...
        public virtual bool Verify(IEnumerable<Transaction> mempool)
        {
            ...
            TransactionResult[] results_destroy = results.Where(p => p.Amount > Fixed10.Zero).ToArray();
            if (results_destroy.Length > 1) return false;
            if (results_destroy.Length == 1 && results_destroy[0].AssetId != Blockchain.UtilityToken.Hash)
                return false;
            if (SystemFee > Fixed10.Zero && (results_destroy.Length == 0 || results_destroy[0].Amount < SystemFee))
                return false;
            TransactionResult[] results_issue = results.Where(p => p.Amount < Fixed10.Zero).ToArray();
            switch (Type)
            {
