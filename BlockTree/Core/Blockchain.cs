...
#pragma warning disable CS0612
        public static readonly RegisterTransaction GoverningToken = new RegisterTransaction
        {
            ...
            Name = "[{\"lang\":\"zh-CN\",\"name\":\"树种\"},{\"lang\":\"en\",\"name\":\"Seed\"}]",
            Amount = Fixed10.FromDecimal(10_000_000_000),
            Precision = 0,
        ...
        public static readonly RegisterTransaction UtilityToken = new RegisterTransaction
        {
            ...
            Name = "[{\"lang\":\"zh-CN\",\"name\":\"肥料\"},{\"lang\":\"en\",\"name\":\"Gas\"}]",
            Amount = Fixed10.FromDecimal(GenerationAmount.Sum(p=>p) * DecrementInterval),
            Precision = 8,
            Owner = ECCurve.Secp256r1.Infinity,
...
#pragma warning restore CS0612
        ...
        public static readonly Block GenesisBlock = new Block
        {
            ...
            Timestamp = (new DateTime(2018, 10, 1, 0, 0, 0, DateTimeKind.Utc)).ToTimestamp(),
            Index = 0,
        ...
        public static Fixed10 CalculateBonus(IEnumerable<CoinReference> inputs, bool ignoreClaimed = true)
        {
        ...
        public static Fixed10 CalculateBonus(IEnumerable<CoinReference> inputs, uint height_end)
        {
            List<SpentCoin> unclaimed = new List<SpentCoin>();
            foreach (var group in inputs.GroupBy(p => p.PrevHash))
            {
        ...
        private static Fixed10 CalculateBonusInternal(IEnumerable<SpentCoin> unclaimed)
        {
            Fixed10 amount_claimed = Fixed10.Zero;
            foreach (var group in unclaimed.GroupBy(p => new { p.StartHeight, p.EndHeight }))
            {
                ...
                amount_claimed += group.Sum(p => p.Value) * amount / 10_000_000_000 ; //整数先乘后除防止误差扩大
            }
                        ...
                        if (out_prev.AssetId.Equals(GoverningToken.Hash))
                        {
                            if (account.Votes.Length > 0)
                            {
                                foreach (ECPoint pubkey in account.Votes)
                                {
                                    ...
                                    if (!validator.Registered && validator.Votes.Equals(Fixed10.Zero))
...
#pragma warning restore CS0612
            ...
            int count = (int)validators_count.Get().Votes.Select((p, i) => new
            {
                Count = i,
                Votes = p
            }).Where(p => p.Votes > Fixed10.Zero).ToArray().WeightedFilter(0.25, 0.75, p => p.Votes.GetData(), (p, w) => new
            {
                p.Count,
                Weight = w
            ...
            ECPoint[] pubkeys = validators.Find().Select(p => p.Value).Where(p => (p.Registered && p.Votes > Fixed10.Zero) || sv.Contains(p.PublicKey)).OrderByDescending(p => p.Votes).ThenBy(p => p.PublicKey).Select(p => p.PublicKey).Take(count).ToArray();
        ...
        protected void ProcessAccountStateDescriptor(StateDescriptor descriptor, DataCache<UInt160, AccountState> accounts, DataCache<ECPoint, ValidatorState> validators, MetaDataCache<ValidatorsCountState> validators_count)
        {
            ...
            {
                case "Votes":
                    Fixed10 balance = account.GetBalance(GoverningToken.Hash);
                    foreach (ECPoint pubkey in account.Votes)
                    {
                        ValidatorState validator = validators.GetAndChange(pubkey);
