...
namespace BlockTree.Core
{
    ...
    public class IssueTransaction : Transaction
    {
        ...
        /// 系统费用
        /// </summary>
        public override Fixed10 SystemFee
        {
            get
            {
                if (Version >= 1) return Fixed10.Zero;
        ...
        public override UInt160[] GetScriptHashesForVerifying()
        {
            ...
            foreach (TransactionResult result in GetTransactionResults().Where(p => p.Amount < Fixed10.Zero))
            {
        ...
        public override bool Verify(IEnumerable<Transaction> mempool)
        {
            ...
            TransactionResult[] results = GetTransactionResults()?.Where(p => p.Amount < Fixed10.Zero).ToArray();
            if (results == null) return false;
            foreach (TransactionResult r in results)
            {
                AssetState asset = Blockchain.Default.GetAssetState(r.AssetId);
                if (asset == null) return false;
                if (asset.Amount < Fixed10.Zero) continue;
                Fixed10 quantity_issued = asset.Available + mempool.OfType<IssueTransaction>().Where(p => p != this).SelectMany(p => p.Outputs).Where(p => p.AssetId == r.AssetId).Sum(p => p.Value);
