...
using System.Text;
using ECPoint = BlockTree.Cryptography.ECC.ECPoint;
...
namespace BlockTree.Wallets
{
    public abstract class Wallet
    {
        ...
        public virtual Coin[] FindUnspentCoins(UInt256 asset_id, Fixed10 amount, params UInt160[] from)
        {
            return FindUnspentCoins(FindUnspentCoins(from), asset_id, amount);
        }

        protected static Coin[] FindUnspentCoins(IEnumerable<Coin> unspents, UInt256 asset_id, Fixed10 amount)
        {
            Coin[] unspents_asset = unspents.Where(p => p.Output.AssetId == asset_id).ToArray();
            Fixed10 sum = unspents_asset.Sum(p => p.Output.Value);
        ...
        public Fixed10 GetAvailable(UInt256 asset_id)
        {
        ...
        public Fixed10 GetBalance(UInt256 asset_id)
        {
        ...
        public T MakeTransaction<T>(T tx, UInt160 from = null, UInt160 change_address = null, Fixed10 fee = default(Fixed10)) where T : Transaction
        {
        ...
        public Transaction MakeTransaction(List<TransactionAttribute> attributes, IEnumerable<TransferOutput> outputs, UInt160 from = null, UInt160 change_address = null, Fixed10 fee = default(Fixed10))
        {
